<template>
    <AuthenticatedLayout>
        ...
        <div class="container py-4">
            <div class="mb-3">
                <Link :href="route('especies.listado')" class="btn btn-outline-secondary btn-sm">
                    ...
                    <div class="col-md-5 col-lg-4 bg-light position-relative">
                        <div v-if="especie.imagen_url" class="h-100 position-relative group-image" style="min-height: 350px;">
                            ...
                            <div class="position-absolute bottom-0 start-0 w-100 p-4 text-white pointer-events-none">
                                <h1 class="display-6 fw-bold mb-0 text-shadow">{{ especie.nombre }}</h1>
                                <span class="badge bg-primary mt-2 shadow-sm">ID: #{{ especie.id }}</span>
                            </div>
                        ...
                        <div v-else class="h-100 d-flex flex-column align-items-center justify-content-center bg-primary bg-opacity-10 text-primary p-5" style="min-height: 300px;">
                            ...
                            <h2 class="h4 fw-bold">{{ especie.nombre }}</h2>
                            <span class="badge bg-primary mt-2">ID: #{{ especie.id }}</span>
                        </div>
                    </div>
                            ...
                            <div class="mb-4 flex-grow-1">
                                ...
                                <div class="bg-light bg-opacity-50 p-4 rounded-3 border border-light">
                                    <p class="text-secondary fs-6 lh-lg mb-0" style="text-align: justify;">
                                        {{ especie.descripcion || 'No hay descripción detallada disponible para esta especie.' }}
                                    </p>
                                </div>
                            </div>
                                ...
                                <div class="d-flex flex-wrap gap-2 m-3">
                                    <template v-if="razas && razas.length > 0">
                                        <Link 
                                            v-for="raza in razas" 
                                            :key="raza.id" 
                                            :href="route('razas.detalle', raza.id)" 
                                        ...
                                        >
                                            <span class="badge bg-primary bg-opacity-10 text-primary border border-primary border-opacity-25 px-3 py-2 rounded-pill hover-elevate transition-all">
                                                <i class="bi bi-tag-fill me-1"></i>{{ raza.nombre }}
                                            </span>
                                        </Link>
                                    </template>
                                    <div v-else class="text-muted small fst-italic">
                                        No hay razas asociadas a esta especie.
                                    </div>
                                </div>
                            </div>
                                ...
                                <div class="col-sm-6">
                                    <div class="d-flex align-items-center gap-3">
                                        ...
                                        <div>
                                            ...
                                            <p class="mb-0 fw-bold text-dark fs-5">{{ new Date(especie.created_at).toLocaleDateString() }}</p>
                                        </div>
                                    </div>
                                </div>
                                <div class="col-sm-6">
                                    <div class="d-flex align-items-center gap-3">
                                        ...
                                        <div>
                                            ...
                                            <p class="mb-0 fw-bold text-dark fs-5">{{ new Date(especie.updated_at).toLocaleDateString() }}</p>
                                        </div>
                                    </div>
                                </div>
                            </div>
