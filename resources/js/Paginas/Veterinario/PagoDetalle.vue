<template>
    <Head :title="`Liquidación - ${veterinario.nombre}`" />
    ...
    <AuthenticatedLayout>
        <template #header>
            <div class="d-flex justify-content-between align-items-center">
                ...
                <Link :href="route('pagos.veterinarios')" class="btn btn-outline-secondary btn-sm rounded-pill px-3 shadow-sm d-print-none">
        ...
        <div class="container py-4">
            ...
            <div class="card border-0 shadow-sm rounded-4 mb-4 bg-white">
                <div class="card-body p-4 d-flex justify-content-between align-items-center">
                    <div>
                        <h4 class="fw-bold mb-1">{{ veterinario.nombre }}</h4>
                        <p class="text-muted mb-0">
                            Periodo: <span class="fw-semibold text-dark">{{ nombreMesActual }} {{ anio_inicial }}</span>
                        </p>
                    </div>
                    <div>
                                ...
                                <tbody>
                                    ...
                                    <tr v-for="item in desglose" :key="item.id">
                                        ...
                                        <td class="text-end pe-4 fw-semibold text-success">
                                            ${{ formatoDinero(item.ganancia_vet) }}
                                        </td>
                                    </tr>
                                </tbody>
                            </table>
...
export default {
    ...
    methods: {
        ...
        async procesarPago() {
            if (!confirm(`¿Estás seguro de registrar el pago de $${this.formatoDinero(this.total)} a ${this.veterinario.nombre}?`)) {
                return;
            }
            ...
            try {
                const response = await axios.post(route('pagos.veterinarios.pagar', this.veterinario.id), {
